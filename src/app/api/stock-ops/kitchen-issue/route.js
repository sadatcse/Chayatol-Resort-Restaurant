...
// GET — paginated kitchen issue records
export async function GET(req) {
  ...
  try {
    ...
    console.error("GET kitchen-issue error:", err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}

// POST — record a kitchen issue
export async function POST(req) {
  ...
  try {
    await dbConnect();
    const { ingredientId, quantity, kitchenName, note, date } = await req.json();
    ...
    await logTransaction({
      ...
      resStatus: 201,
      user: auth.user,
      details: `Kitchen issue: ${qty} units of ingredient ${ingredientId} → ${kitchenName}`,
    });

    return NextResponse.json(movement, { status: 201 });
  } catch (err) {
    console.error("POST kitchen-issue error:", err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
