...
const CashFlowReport = () => {
  ...
  const {
    ...
    documentTitle: `Cash_Flow_${monthLabel}_${selectedYear}`,
  });
  ...
  const handleExportExcel = () => {
    ...
    const rows = [
      ...
      { LineItem: "  General Overhead Expenses Paid", Amount: cashFlow.outflow.general },
      { LineItem: "  Supplier Payments Paid (Vendor ledger)", Amount: cashFlow.outflow.vendorPayments },
      { LineItem: "  Guest Booking Cancellation Payouts", Amount: cashFlow.outflow.refunds },
      { LineItem: "TOTAL CASH OUTFLOW (B)", Amount: cashFlow.outflow.total },
      { LineItem: "", Amount: "" },
      { LineItem: "NET OPERATING CASH FLOW (A - B)", Amount: cashFlow.netCashFlow }
    ];

    exportToExcel(rows, `Cash_Flow_${monthLabel}_${selectedYear}`, "Cash Flow");
  };
  ...
  const handleExportCsv = () => {
    ...
    exportToCsv(rows, `Cash_Flow_${monthLabel}_${selectedYear}`);
  };
    ...
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      <div className="print:hidden">
        <SectionHeader
          ...
          subtitle="Cash-basis statement of cash flows tracking actual physical cash receipts vs cash outflows."
        />
      ...
      {isLoading ? (
        ...
        <div className="max-w-4xl mx-auto bg-white dark:bg-brand-charcoal/30 p-8 sm:p-12 rounded-3xl border border-brand-beige/50 dark:border-brand-dark-grey/50 shadow-md">
          ...
          <div className="text-center border-b-2 border-brand-primary pb-6 mb-8">
            ...
            <h2 className="text-md font-bold uppercase tracking-wider text-brand-sage mt-1">Statement of Cash Flows</h2>
            <div className="text-xs font-semibold mt-3 text-brand-charcoal dark:text-brand-offwhite">
              ...
              <p className="text-[10px] font-bold text-brand-sage mt-0.5">(Direct Cash Accounting Method)</p>
            </div>
          </div>

          <div className="space-y-8 text-sm">
            <div>
              <h3 className="font-bold text-xs uppercase tracking-widest text-brand-primary border-b border-brand-primary/20 pb-1 mb-3">Cash Inflows (Operating Receipts)</h3>
              <div className="space-y-2.5">
                <div className="flex justify-between pl-4">
                  <span>Guest Booking Cash Payments</span>
                  <span className="font-mono font-bold">৳{cashFlow.inflow.room.toLocaleString()}</span>
                </div>
                <div className="flex justify-between pl-4">
            ...
            <div>
              <h3 className="font-bold text-xs uppercase tracking-widest text-brand-primary border-b border-brand-primary/20 pb-1 mb-3">Cash Outflows (Operating Payments)</h3>
              <div className="space-y-2.5">
                <div className="flex justify-between pl-4">
                  <span>General Overhead Expenses Paid</span>
                  <span className="font-mono font-bold text-red-500">৳{cashFlow.outflow.general.toLocaleString()}</span>
                </div>
                <div className="flex justify-between pl-4">
                  <span>Supplier Ledger Payments Paid (Vendor payments)</span>
                  <span className="font-mono font-bold text-red-500">৳{cashFlow.outflow.vendorPayments.toLocaleString()}</span>
                </div>
                <div className="flex justify-between pl-4">
                  <span>Guest Booking Cancellation Payouts</span>
                  <span className="font-mono font-bold text-red-500">৳{cashFlow.outflow.refunds.toLocaleString()}</span>
                </div>

                <div className="flex justify-between font-black text-brand-charcoal dark:text-brand-offwhite border-t border-brand-beige/50 dark:border-brand-beige/10 pt-2">
                  <span>TOTAL OPERATIONAL CASH OUTFLOW (B)</span>
                  <span className="font-mono">৳{cashFlow.outflow.total.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-brand-primary/5 dark:bg-brand-charcoal p-6 rounded-2xl border border-brand-primary/10 dark:border-brand-beige/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold uppercase tracking-widest text-brand-sage">Net Cash Position</h4>
                  <div className="text-xl font-black text-brand-black dark:text-brand-offwhite uppercase tracking-wider">Net Cash Flow (A - B)</div>
                </div>
              </div>
              <div className={`text-2xl font-black font-mono ${cashFlow.netCashFlow >= 0 ? "text-green-600" : "text-red-500"}`}>
            ...
            <div className="flex justify-between items-center text-[10px] text-brand-sage font-bold uppercase tracking-widest pt-8 border-t border-dashed border-brand-beige/50 dark:border-brand-beige/10">
              <span>Prepared By: Chayatol Finance</span>
          ...
          >
            <div style={{ fontSize: "12px" }}>
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ borderBottom: "1px solid #000", paddingBottom: "4px", textTransform: "uppercase", fontWeight: "bold", fontSize: "12px", color: "#000" }}>Cash Inflows (Operating Receipts)</h3>
                <table className="print-table" style={{ width: "100%", borderCollapse: "collapse", marginTop: "8px" }}>
                  <tbody>
                    <tr>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd" }}>Guest Booking Cash Payments</td>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace", fontWeight: "bold" }}>৳{printData.cashFlow?.inflow?.room?.toLocaleString()}</td>
                    </tr>
                    <tr>
                    ...
                    <tr style={{ fontWeight: "bold", backgroundColor: "#f3f4f6" }}>
                      ...
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace" }}>৳{printData.cashFlow?.inflow?.total?.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div style={{ marginBottom: "20px" }}>
                <h3 style={{ borderBottom: "1px solid #000", paddingBottom: "4px", textTransform: "uppercase", fontWeight: "bold", fontSize: "12px", color: "#000" }}>Cash Outflows (Operating Payments)</h3>
                <table className="print-table" style={{ width: "100%", borderCollapse: "collapse", marginTop: "8px" }}>
                  <tbody>
                    <tr>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd" }}>General Overhead Expenses Paid</td>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace", fontWeight: "bold", color: "#ef4444" }}>৳{printData.cashFlow?.outflow?.general?.toLocaleString()}</td>
                    </tr>
                    <tr>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd" }}>Supplier Ledger Payments Paid (Vendor payments)</td>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace", fontWeight: "bold", color: "#ef4444" }}>৳{printData.cashFlow?.outflow?.vendorPayments?.toLocaleString()}</td>
                    </tr>
                    <tr>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd" }}>Guest Booking Cancellation Payouts</td>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace", fontWeight: "bold", color: "#ef4444" }}>৳{printData.cashFlow?.outflow?.refunds?.toLocaleString()}</td>
                    </tr>
                    <tr style={{ fontWeight: "bold", backgroundColor: "#f3f4f6" }}>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd" }}>TOTAL OPERATIONAL CASH OUTFLOW (B)</td>
                      <td style={{ padding: "6px 8px", border: "1px solid #ddd", textAlign: "right", fontFamily: "monospace" }}>৳{printData.cashFlow?.outflow?.total?.toLocaleString()}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
