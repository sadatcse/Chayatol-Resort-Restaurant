...
const RolesPage = () => {
  ...
  const { roles, totalPages, totalItems, isLoading, refetch } = useRoles(
    currentPage,
    itemsPerPage,
    debouncedSearchTerm
  );
  ...
  const openModal = (roleToEdit = null) => {
    if (roleToEdit) {
      setEditId(roleToEdit._id);
      setFormData({ userrole: roleToEdit.userrole });
    } else {
      setEditId(null);
  ...
  const handleAddOrEdit = async () => {
    if (editId ? !canEdit : !canAdd) {
      Swal.fire({
        title: "Access Denied",
        text: `You do not have permission to ${editId ? "update" : "create"} user roles.`,
        icon: "error",
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/userrole/update/${editId}`, formData);
      } else {
        await axiosSecure.post("/userrole/post", formData);
      }
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `User Role successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
      ...
      Swal.fire({
        title: "Action Failed",
        text: error.response?.data?.message || "Failed to save user role.",
        icon: "error",
        confirmButtonColor: "#346E36",
      });
    } finally {
  ...
  const handleDelete = (id) => {
    if (!canDelete) {
      Swal.fire({
        title: "Access Denied",
        text: "You do not have permission to delete user roles.",
        icon: "error",
        confirmButtonColor: "#8C5A35",
      });
      return;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosSecure.delete(`/userrole/delete/${id}`);
          Swal.fire({
            title: "Deleted!",
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      <SectionHeader
        ...
        subtitle="Configure system access roles and user types."
      >
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          {canAdd && (
            ...
            >
              ...
              <span className="uppercase tracking-widest text-xs font-bold">New Role</span>
            </button>
          )}
        </div>
      </SectionHeader>
              ...
              <table className="table w-full">
                <thead className="bg-brand-primary text-white font-bold uppercase tracking-widest text-[10px] border-b border-brand-beige dark:border-brand-beige/20">
                  <tr>
                    ...
                    <th className="py-5">User Role</th>
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20">
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest">
                {editId ? 'Update User Role' : 'Create User Role'}
              </h3>
            ...
            <div className="p-8">
              <div className="form-control w-full">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ userrole: e.target.value })}
