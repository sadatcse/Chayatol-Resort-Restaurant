...
const FoodCategoriesPage = () => {
  ...
  const { categories, totalPages, totalItems, isLoading, refetch } = useFoodCategories(
    currentPage,
    itemsPerPage,
    debouncedSearchTerm
  );
  ...
  const openModal = (catToEdit = null) => {
    if (catToEdit) {
      ...
      setFormData({
        ...
        serial: catToEdit.serial !== undefined ? catToEdit.serial : "",
  ...
  const handleAddOrEditCategory = async () => {
    if (editId ? !canEdit : !canAdd) {
      Swal.fire({
        title: "Access Denied",
        text: `You do not have permission to ${editId ? "update" : "create"} food categories.`,
        icon: "error",
    ...
    // 2. Validation: Serial Number
    const serialVal = parseInt(formData.serial);
    if (formData.serial === "" || isNaN(serialVal) || serialVal < 0) {
      Swal.fire({
        title: "Validation Error",
        text: "Please enter a valid display serial number (0 or positive integer).",
        icon: "warning",
        confirmButtonColor: "#346E36"
      });
      return;
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/category/update/${editId}`, payload);
      } else {
        await axiosSecure.post("/category/post", payload);
      }
      // Invalidate local storage cache to force hook reload
      localStorage.removeItem("categories");
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `Food Category successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
        confirmButtonColor: "#346E36",
      });
    } catch (error) {
  ...
  const handleDelete = (id) => {
    if (!canDelete) {
      Swal.fire({
        title: "Access Denied",
        text: "You do not have permission to delete food categories.",
        icon: "error",
        confirmButtonColor: "#8C5A35",
      });
      return;
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this food category record!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axiosSecure.delete(`/category/delete/${id}`);
          localStorage.removeItem("categories");
          await refetch();
          Swal.fire({
            title: "Deleted!",
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      ...
      <SectionHeader 
        ...
        subtitle="Manage food classifications, set displaying orders, and activate/deactivate menus."
      >
      ...
      <div className="flex flex-wrap justify-between items-center bg-white dark:bg-brand-charcoal p-4 rounded-2xl shadow-sm border border-brand-beige dark:border-brand-beige/20 mb-6 gap-4">
        ...
        {canAdd && (
          <div className="flex gap-2">
            <button onClick={() => openBulkUiModal()} className="btn bg-white text-brand-charcoal hover:bg-gray-100 border border-gray-200 btn-sm rounded-full shadow-sm gap-2 px-6 h-10 cursor-pointer">
            ...
            <div className="overflow-x-auto">
              <table className="table w-full">
                <thead className="bg-brand-primary text-white font-bold uppercase tracking-widest text-[10px] border-b border-brand-beige dark:border-brand-beige/20">
                  <tr>
                    <th className="pl-8 py-5 w-24">Display Serial</th>
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20 animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest">
                {editId ? 'Update Food Category' : 'Create Food Category'}
              </h3>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="form-control w-full">
                <label className="label py-1"><span className="label-text text-xs font-bold text-brand-sage uppercase tracking-widest">Display Serial Number *</span></label>
                <input
                  type="number"
              ...
              <div className="form-control w-full p-4 bg-brand-offwhite dark:bg-brand-charcoal/50 border border-brand-beige dark:border-brand-beige/20 rounded-2xl flex flex-row items-center justify-between">
                <div>
                  ...
                  <span className="text-[10px] text-brand-sage/80 block mt-1">If inactive, this category will not appear in ordering screens.</span>
                </div>
                <input 
                  type="checkbox" 
