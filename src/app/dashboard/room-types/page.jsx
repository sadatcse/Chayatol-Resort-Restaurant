...
const RoomTypesPage = () => {
  ...
  const handleAddOrEditRoomType = async () => {
    if (!formData.name || !formData.name.trim()) {
      Swal.fire({
        ...
        text: "Please provide the room type name.",
        icon: "warning",
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/room-type/${editId}`, payload);
      } else {
        await axiosSecure.post("/room-type", payload);
      }
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `Room Type successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
        confirmButtonColor: "#346E36",
      });
    } catch (error) {
  ...
  const handleDelete = (id, typeName) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `You won't be able to revert this room type: ${typeName}!`,
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axiosSecure.delete(`/room-type/${id}`);
          await refetch();
          Swal.fire({
            title: "Deleted!",
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      ...
      <SectionHeader
        ...
        subtitle="Manage resort room types and categories for rooms configuration."
      >
                    ...
                    {roomTypes.length === 0 ? (
                      ...
                      roomTypes.map((type, index) => (
                        ...
                        >
                          ...
                          <td className="py-4 text-brand-sage dark:text-brand-beige">
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20 animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest">
                {editId ? 'Update Room Type' : 'Create Room Type'}
              </h3>
            ...
            <div className="p-8 space-y-4">
              <div className="form-control w-full">
                <label className="label py-1"><span className="label-text text-xs font-bold text-brand-sage uppercase tracking-widest">Room Type Name *</span></label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="form-control w-full">
                ...
                <textarea
                  ...
                  className="textarea textarea-bordered border-brand-primary dark:border-brand-primary/50 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary w-full bg-white dark:bg-brand-charcoal/50 text-brand-charcoal dark:text-brand-offwhite h-24"
                  placeholder="Details about this room type..."
                />
              </div>

              <div className="form-control w-full">
                <label className="label cursor-pointer flex items-center justify-between py-2">
                  <span className="label-text text-xs font-bold text-brand-sage uppercase tracking-widest">Is Active</span>
                  <input
                    type="checkbox"
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              ...
              <button onClick={handleAddOrEditRoomType} className="btn bg-brand-primary text-white hover:bg-brand-secondary border-none font-bold uppercase tracking-widest text-xs px-8 shadow-md" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
