...
export default function ResortPOSPage() {
  ...
  const [vatValue, setVatValue] = useState(0); // Optional global VAT %
  const [sdValue, setSdValue] = useState(0); // Optional global SD %
  const [serviceChargeValue, setServiceChargeValue] = useState(0); // Optional global SC %
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const roomsRes = await axiosSecure.get("/room").catch(() => ({ data: [] }));
  ...
  const vat = (subTotalAfterDiscount * vatValue) / 100;
  const sdAmt = (subTotalAfterDiscount * sdValue) / 100;
  const serviceChargeAmt = (subTotalAfterDiscount * serviceChargeValue) / 100;
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex gap-4">
      {/* MIDDLE: Services Grid & Top Categories */}
        ...
        <div className="flex-1 bg-white dark:bg-brand-charcoal rounded-xl shadow-sm p-4 overflow-y-auto custom-scrollbar border border-gray-100 dark:border-gray-800">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                ...
                <div className="h-32 w-full bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                   {service.image ? (
                      ...
                      <div className="flex h-full items-center justify-center text-gray-400">
                         <MdOutlineTableRestaurant size={40} className="opacity-20" />
                      </div>
                   )}
            ...
            {filteredServices.length === 0 && (
              <div className="col-span-full py-10 text-center text-gray-400 font-medium">No services found.</div>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="p-4 flex flex-col gap-4 flex-1 overflow-hidden">
            ...
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 border border-gray-100 dark:border-gray-700 shrink-0">
               ...
               <div className="grid grid-cols-2 gap-2 mb-3">
                  <div>
                    <label className="text-xs font-bold text-gray-500 mb-1 block">VAT (%)</label>
                    <input type="number" value={vatValue} onChange={e => setVatValue(Number(e.target.value))} className="w-full border border-gray-200 rounded-lg p-2 text-sm dark:bg-gray-800 dark:border-gray-700" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 mb-1 block">SD (%)</label>
                    <input type="number" value={sdValue} onChange={e => setSdValue(Number(e.target.value))} className="w-full border border-gray-200 rounded-lg p-2 text-sm dark:bg-gray-800 dark:border-gray-700" />
                  </div>
               </div>
