...
const RestaurantTablesPage = () => {
  ...
  const { restaurantTables, totalPages, totalItems, isLoading, refetch } = useRestaurantTables(
    currentPage,
    itemsPerPage,
    debouncedSearchTerm
  );
  ...
  const openModal = (tableToEdit = null) => {
    if (tableToEdit) {
      setEditId(tableToEdit._id);
      setFormData({
  ...
  const handleAddOrEditTable = async () => {
    ...
    if (!formData.tableName || !formData.tableName.trim()) {
      Swal.fire({
        ...
        text: "Please provide the restaurant table name.",
        icon: "warning",
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/restauranttable/update/${editId}`, payload);
      } else {
        await axiosSecure.post("/restauranttable/post", payload);
      }
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `Restaurant Table successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
        confirmButtonColor: "#346E36",
      });
    } catch (error) {
      Swal.fire({
        title: "Action Failed",
        text: error.response?.data?.message || "Failed to save restaurant table.",
        icon: "error",
        confirmButtonColor: "#346E36",
      });
    } finally {
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this restaurant table record!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire("Error!", "Failed to delete restaurant table.", "error");
        }
      }
    });
  };
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      ...
      <SectionHeader
        ...
        subtitle="Manage resort restaurant seating arrangements, layouts, and table details."
      >
              ...
              <table className="table w-full">
                <thead className="bg-brand-primary text-white font-bold uppercase tracking-widest text-[10px] border-b border-brand-beige dark:border-brand-beige/20">
                  <tr>
                    ...
                    <th className="py-5">Table Name</th>
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20 animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest">
                {editId ? 'Update Restaurant Table' : 'Create Restaurant Table'}
              </h3>
            ...
            <div className="p-8 space-y-6">
              <div className="form-control w-full">
                <label className="label py-1"><span className="label-text text-xs font-bold text-brand-sage uppercase tracking-widest">Table Name *</span></label>
                <input
                  type="text"
                  ...
                  placeholder="e.g. Table 01 / Table VIP"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              ...
              <button onClick={handleAddOrEditTable} className="btn bg-brand-primary text-white hover:bg-brand-secondary border-none font-bold uppercase tracking-widest text-xs px-8 shadow-md" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
