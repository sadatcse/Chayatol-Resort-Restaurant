...
import { FiEdit, FiTrash2, FiX, FiSearch, FiPlus, FiCreditCard, FiCheckCircle } from "react-icons/fi";
...
const PaymentTypesPage = () => {
  ...
  const { paymentTypes, totalPages, totalItems, isLoading, refetch } = usePaymentTypes(
    currentPage,
    itemsPerPage,
    debouncedSearchTerm
  );
  ...
  const openModal = (paymentToEdit = null) => {
    if (paymentToEdit) {
      setEditId(paymentToEdit._id);
      setFormData({
  ...
  const handleAddOrEditPayment = async () => {
    if (editId ? !canEdit : !canAdd) {
      Swal.fire({
        title: "Access Denied",
        text: `You do not have permission to ${editId ? "update" : "create"} payment types.`,
        icon: "error",
    ...
    if (!formData.name || !formData.name.trim()) {
      Swal.fire({
        ...
        text: "Please provide the payment type name.",
        icon: "warning",
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/paymenttype/update/${editId}`, payload);
      } else {
        await axiosSecure.post("/paymenttype/post", payload);
      }
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `Payment Type successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
        confirmButtonColor: "#346E36",
      });
    } catch (error) {
      Swal.fire({
        title: "Action Failed",
        text: error.response?.data?.message || "Failed to save payment type.",
        icon: "error",
        confirmButtonColor: "#346E36",
      });
    } finally {
  ...
  const handleDelete = (id) => {
    if (!canDelete) {
      Swal.fire({
        title: "Access Denied",
        text: "You do not have permission to delete payment types.",
        icon: "error",
        confirmButtonColor: "#8C5A35",
      });
      return;
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this payment type record!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axiosSecure.delete(`/paymenttype/delete/${id}`);
          await refetch();
          Swal.fire({
            title: "Deleted!",
          ...
          Swal.fire("Error!", "Failed to delete payment type.", "error");
        }
      }
    });
  };
  ...
  const handleImageUploadComplete = (url) => {
    if (url) {
      ...
      Swal.fire({
        ...
        text: 'Click save changes to apply this branding image.',
        timer: 1500,
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      ...
      <SectionHeader 
        title="Payment Types" 
        subtitle="Manage accepted resort payment methods, branding icons, and gateways."
      >
      ...
      <div className="stats shadow-sm bg-white dark:bg-brand-charcoal w-full mb-8 border border-brand-beige dark:border-brand-beige/20 rounded-2xl overflow-hidden hidden md:flex animate-fade-in">
        <div className="stat place-items-center py-6">
          ...
          <div className="stat-title text-brand-sage font-bold uppercase tracking-wider text-[10px] mt-2">Active Methods</div>
      ...
      <div className="flex flex-wrap justify-between items-center bg-white dark:bg-brand-charcoal p-4 rounded-2xl shadow-sm border border-brand-beige dark:border-brand-beige/20 mb-6 gap-4">
        ...
        {canAdd && (
          <button onClick={() => openModal()} className="btn bg-brand-primary text-white hover:bg-brand-secondary border-none btn-sm rounded-full shadow-md gap-2 px-6 h-10 cursor-pointer">
            ...
            <span className="uppercase tracking-widest text-xs font-bold">New Payment Method</span>
          </button>
        )}
      </div>
              ...
              <table className="table w-full">
                <thead className="bg-brand-primary text-white font-bold uppercase tracking-widest text-[10px] border-b border-brand-beige dark:border-brand-beige/20">
                  <tr>
                    ...
                    <th className="py-5">Payment Icon</th>
                    <th className="py-5">Payment Method Name</th>
                        ...
                        >
                          ...
                          <td className="py-4">
                            <div className="avatar">
                              <div className="w-10 h-10 rounded-lg ring-1 ring-brand-primary/20 bg-brand-offwhite dark:bg-brand-offwhite/5 flex items-center justify-center text-brand-primary">
                                {payment.image ? (
                                  <img src={payment.image} alt={payment.name} className="object-contain p-1" />
                                ) : (
                          ...
                          <td className="pr-8 py-4">
                            <div className="flex justify-center items-center gap-2">
                              {canEdit || canDelete ? (
                                <>
                                  {canEdit && (
                                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => openModal(payment)} className="btn btn-sm btn-circle btn-ghost text-brand-sage hover:text-brand-primary hover:bg-brand-primary/10 transition-colors shadow-none cursor-pointer" title="Edit Payment Type">
                                      <FiEdit size={16} />
                                    </motion.button>
                                  )}
                                  {canDelete && (
                                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleDelete(payment._id)} className="btn btn-sm btn-circle btn-ghost text-brand-sage hover:text-red-500 hover:bg-red-50 transition-colors shadow-none cursor-pointer" title="Delete Payment Type">
                                      <FiTrash2 size={16} />
                                    </motion.button>
                                  )}
                                </>
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20 animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest">
                {editId ? 'Update Payment Type' : 'Create Payment Type'}
              </h3>
            ...
            <div className="p-8 space-y-6">
              <div className="form-control w-full">
                <label className="label py-1"><span className="label-text text-xs font-bold text-brand-sage uppercase tracking-widest">Payment Method Name *</span></label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div className="p-4 bg-brand-offwhite dark:bg-brand-charcoal/50 border border-brand-beige dark:border-brand-beige/20 rounded-2xl">
                <span className="block text-xs font-bold text-brand-sage uppercase tracking-widest mb-3">Branding Icon/Logo</span>
                <div className="flex items-center gap-4">
                  <div className="avatar">
                    <div className="w-14 h-14 rounded-lg bg-white dark:bg-brand-charcoal border border-brand-beige dark:border-brand-beige/20 flex items-center justify-center text-brand-primary shadow-inner">
                      {formData.image ? (
                        <img src={formData.image} alt="Payment Preview" className="object-contain p-1" />
                      ) : (
                  ...
                  <div className="grow">
                    <ImageUpload setImageUrl={handleImageUploadComplete} label="Select branding icon" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              ...
              <button onClick={handleAddOrEditPayment} className="btn bg-brand-primary text-white hover:bg-brand-secondary border-none font-bold uppercase tracking-widest text-xs px-8 shadow-md" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
