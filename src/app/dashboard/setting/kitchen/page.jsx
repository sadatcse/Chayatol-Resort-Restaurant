...
const KitchensPage = () => {
  ...
  const { kitchens, totalPages, totalItems, isLoading, refetch } = useKitchens(
    currentPage,
    itemsPerPage,
    debouncedSearchTerm
  );
  ...
  const openModal = (kitchenToEdit = null) => {
    if (kitchenToEdit) {
      setEditId(kitchenToEdit._id);
      setFormData({
        name: kitchenToEdit.name || ""
      });
    } else {
      setEditId(null);
  ...
  const handleAddOrEditKitchen = async () => {
    ...
    if (!formData.name || !formData.name.trim()) {
      Swal.fire({
        ...
        text: "Please provide the kitchen name.",
        icon: "warning",
    ...
    try {
      if (editId) {
        await axiosSecure.put(`/kitchen/update/${editId}`, payload);
      } else {
        await axiosSecure.post("/kitchen/post", payload);
      }
      await refetch();
      closeModal();
      Swal.fire({
        title: "Success",
        text: `Kitchen successfully ${editId ? "updated" : "created"}.`,
        icon: "success",
        confirmButtonColor: "#346E36",
      });
    } catch (error) {
      Swal.fire({
        title: "Action Failed",
        text: error.response?.data?.message || "Failed to save kitchen details.",
        icon: "error",
        confirmButtonColor: "#346E36",
      });
    } finally {
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: "You won't be able to revert this kitchen record!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          await axiosSecure.delete(`/kitchen/delete/${id}`);
          await refetch();
          Swal.fire({
            title: "Deleted!",
            text: "Kitchen has been deleted.",
            icon: "success",
            confirmButtonColor: "#346E36",
          });
        } catch (error) {
          Swal.fire("Error!", "Failed to delete kitchen.", "error");
        }
      }
    });
  };
  ...
  return (
    <div className="p-4 sm:p-8 min-h-screen bg-brand-offwhite dark:bg-brand-charcoal font-sans text-brand-charcoal dark:text-brand-offwhite animate-scale-in">
      ...
      <SectionHeader
        ...
        subtitle="Manage resort kitchens, dining outlets, and food preparation sections."
      >
              ...
              <table className="table w-full">
                <thead className="bg-brand-primary text-white font-bold uppercase tracking-widest text-[10px] border-b border-brand-beige dark:border-brand-beige/20">
                  <tr>
                    ...
                    <th className="py-5">Kitchen Name</th>
          ...
          <div className="modal-box bg-white dark:bg-brand-charcoal p-0 overflow-hidden max-w-md rounded-2xl shadow-2xl border border-brand-beige/20 dark:border-brand-beige/20 animate-scale-in">
            ...
            <div className="flex justify-between items-center p-6 border-b border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              <h3 className="font-bold text-lg text-brand-black dark:text-brand-offwhite uppercase tracking-widest flex items-center gap-2">
                <MdKitchen className="text-brand-primary" /> {editId ? 'Update Kitchen' : 'Create Kitchen'}
              </h3>
            ...
            <div className="p-8 space-y-6">
              <div className="form-control w-full">
                ...
                <input
                  ...
                  placeholder="e.g. Main Kitchen / Bakery"
                  autoFocus
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-brand-beige dark:border-brand-beige/20 bg-brand-offwhite dark:bg-brand-charcoal/50">
              ...
              <button onClick={handleAddOrEditKitchen} className="btn bg-brand-primary text-white hover:bg-brand-secondary border-none font-bold uppercase tracking-widest text-xs px-8 shadow-md" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
