...
const useKitchenIssue = (currentPage = 1, itemsPerPage = 10, searchTerm = "", from = null, to = null) => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axiosSecure.get(`/stock-ops/kitchen-issue?${params.toString()}`);
      setRecords(data.data || []);
      setTotalPages(data.pagination?.totalPages || 1);
      setTotalItems(data.pagination?.totalDocuments || 0);
    } catch (err) {
      console.error("Error fetching kitchen issue records:", err);
    } finally {
      setIsLoading(false);
    }
...
export default useKitchenIssue;
