...
export default function ChargeSettingsPage() {
    ...
    try {
      ...
      if (res.data) {
        setSettings(prev => ({
          ...prev,
          ...res.data,
          vat: { ...prev.vat, ...res.data.vat },
          sc: { ...prev.sc, ...res.data.sc },
          sd: { ...prev.sd, ...res.data.sd },
          deliveryCharge: { ...prev.deliveryCharge, ...res.data.deliveryCharge }
        }));
      }
    } catch (error) {
      console.error(error);
  ...
  const updateSd = (field, value) => {
    setSettings(prev => ({ ...prev, sd: { ...prev.sd, [field]: value } }));
  };

  const updateSdApp = (appField, value) => {
    setSettings(prev => ({
      ...prev,
      sd: { ...prev.sd, applicability: { ...prev.sd.applicability, [appField]: value } }
    }));
  };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="card bg-brand-white dark:bg-brand-charcoal shadow-xl border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden p-6">
            ...
            <div className={`grid grid-cols-2 gap-3 transition-opacity ${!settings.vat?.customApplicability && 'opacity-50 pointer-events-none'}`}>
              {["Dine In", "Takeaway", "Delivery", "Room Service", "Foodpanda", "Foodi", "Pathao"].map(type => (
          ...
          <div className="card bg-brand-white dark:bg-brand-charcoal shadow-xl border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <input type="checkbox" className="toggle toggle-sm bg-brand-primary" checked={settings.sd?.enabled} onChange={e => updateSd("enabled", e.target.checked)} />
            </div>

            <div className="flex bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden mb-6">
              <input type="number" min="0" value={settings.sd?.value === 0 ? "" : settings.sd?.value} onChange={e => updateSd("value", Number(e.target.value))} className="w-full p-2.5 text-sm outline-none bg-transparent" placeholder="0" />
              <div className="bg-gray-100 dark:bg-gray-700 border-l border-gray-200 dark:border-gray-700 px-3 flex items-center justify-center text-xs font-bold text-gray-500">
                %
              </div>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <span className="font-bold text-sm text-brand-charcoal dark:text-gray-200">Custom Applicability</span>
              <input type="checkbox" className="toggle toggle-sm bg-brand-primary" checked={settings.sd?.customApplicability} onChange={e => updateSd("customApplicability", e.target.checked)} />
            </div>

            <div className={`grid grid-cols-2 gap-3 transition-opacity ${!settings.sd?.customApplicability && 'opacity-50 pointer-events-none'}`}>
              {["Dine In", "Takeaway", "Delivery", "Room Service", "Foodpanda", "Foodi", "Pathao"].map(type => (
                <div key={type} className="flex justify-between items-center border border-gray-100 dark:border-gray-700 p-2 rounded-lg">
                  <span className="text-[10px] font-bold text-gray-500 uppercase">{type}</span>
                  <input type="checkbox" className="toggle toggle-xs bg-brand-primary" checked={settings.sd?.applicability?.[type]} onChange={e => updateSdApp(type, e.target.checked)} />
                </div>
              ))}
            </div>
          </div>
